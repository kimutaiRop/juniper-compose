...
pub fn type_to_owned<'a>(ty: &Type<'a>) -> Type<'static> {
    match ty {
        ...
        Type::List(inner, position) => Type::List(Box::new(type_to_owned(inner)), *position),
        Type::NonNullList(inner, position) => Type::NonNullList(Box::new(type_to_owned(inner)), *position),
    }
}